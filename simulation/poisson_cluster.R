...
Log_lik_int_cluster_poi <- array(rep(0,K_max*n_setting*3),dim = c(n_setting,K_max,3))
Loss_cluster_poi <- array(rep(0,K_max*n_setting*3),dim = c(n_setting,K_max,3))
...
for(i in 1:n_setting){
  ...
  for(k in K_true:K_true){
    ...
    Log_lik_int_cluster_poi[i,k,1] <- result[[1]]
    Loss_cluster_poi[i,k,1] <- result[[2]]
    print(c(i,k,result[[2]]))
  }
}
...
for(i in 1:n_setting){
  ...
  for(k in K_true:K_true){
    ...
    Log_lik_int_cluster_poi[i,k,2] <- result[[1]]
    Loss_cluster_poi[i,k,2] <- result[[2]]
    print(c(i,k,result[[2]]))
  }
}
...
for(i in 1:n_setting){
  ...
  for(k in K_true:K_true){
    ...
    Log_lik_int_cluster_poi[i,k,3] <- result[[1]]
    Loss_cluster_poi[i,k,3] <- result[[2]]
    print(c(i,k,result[[2]]))
  }
}
...
rm(event)
rm(transaction)
save.image(paste0("poisson_cluster_",id,".RData"))